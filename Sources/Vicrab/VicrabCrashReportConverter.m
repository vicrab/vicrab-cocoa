...
@implementation VicrabCrashReportConverter
...
- (VicrabEvent *)convertReportToEvent {
    VicrabEvent *event = [[VicrabEvent alloc] initWithLevel:kVicrabSeverityFatal];
    if ([self.report[@"report"][@"timestamp"] isKindOfClass:NSNumber.class]) {
        ...
        event.timestamp = [NSDate vicrab_fromIso8601String:self.report[@"report"][@"timestamp"]];
    }
...
- (VicrabUser *_Nullable)convertUser {
    VicrabUser *user = nil;
    if (nil != self.userContext[@"user"]) {
        user = [[VicrabUser alloc] init];
...
- (void)addAppContext:(VicrabContext *)context {
...
- (void)addDeviceContext:(VicrabContext *)context {
...
- (void)addOsContext:(VicrabContext *)context {
...
- (VicrabThread *_Nullable)threadAtIndex:(NSInteger)threadIndex stripCrashedStacktrace:(BOOL)stripCrashedStacktrace {
    if (threadIndex >= [self.threads count]) {
        return nil;
    }
    NSDictionary *threadDictionary = [self.threads objectAtIndex:threadIndex];

    VicrabThread *thread = [[VicrabThread alloc] initWithThreadId:threadDictionary[@"index"]];
...
- (VicrabFrame *)stackFrameAtIndex:(NSInteger)frameIndex inThreadIndex:(NSInteger)threadIndex {
...
- (NSArray<VicrabFrame *> *)stackFramesForThreadIndex:(NSInteger)threadIndex {
...
- (VicrabStacktrace *)stackTraceForThreadIndex:(NSInteger)threadIndex {
    NSArray<VicrabFrame *> *frames = [self stackFramesForThreadIndex:threadIndex];
    VicrabStacktrace *stacktrace = [[VicrabStacktrace alloc] initWithFrames:frames
                                                                  registers:[self registersForThreadIndex:threadIndex]];
    [stacktrace fixDuplicateFrames];
    return stacktrace;
}

- (VicrabThread *_Nullable)crashedThread {
    return [self threadAtIndex:self.crashedThreadIndex stripCrashedStacktrace:NO];
}

- (NSArray<VicrabDebugMeta *> *)convertDebugMeta {
    NSMutableArray<VicrabDebugMeta *> *result = [NSMutableArray new];
...
- (NSArray<VicrabException *> *_Nullable)convertExceptions {
    ...
    if ([exceptionType isEqualToString:@"nsexception"]) {
        ...
        exception = [[VicrabException alloc] initWithValue:[NSString stringWithFormat:@"Exception %@, Code %@, Subcode %@",
        ...
        exception = [[VicrabException alloc] initWithValue:[NSString stringWithFormat:@"Signal %@, Code %@",
        ...
        exception = [[VicrabException alloc] initWithValue:exceptionReason
                                                      type:self.exceptionContext[@"user_reported"][@"name"]];

        NSRange match = [exceptionReason rangeOfString:@":"];
        if (match.location != NSNotFound) {
            exception = [[VicrabException alloc] initWithValue:[[exceptionReason substringWithRange:NSMakeRange(match.location + match.length, (exceptionReason.length - match.location) - match.length)] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]
...
- (void)enhanceValueFromNotableAddresses:(VicrabException *)exception {
    // Gatekeeper fixes https://github.com/getvicrab/vicrab-cocoa/issues/231
...
- (VicrabMechanism *_Nullable)extractMechanism {
    VicrabMechanism *mechanism = [[VicrabMechanism alloc] initWithType:[self.exceptionContext objectForKey:@"type"]];
...
- (NSArray *)convertThreads {
    ...
    for (NSInteger threadIndex = 0; threadIndex < (NSInteger) self.threads.count; threadIndex++) {
        VicrabThread *thread = [self threadAtIndex:threadIndex stripCrashedStacktrace:YES];
        if (thread) {
