...
@interface  VicrabCrashDoctorParam: NSObject
...
@implementation VicrabCrashDoctorParam
...
@interface VicrabCrashDoctorFunctionCall: NSObject
...
@implementation VicrabCrashDoctorFunctionCall
...
- (NSString*) descriptionForObjCCall
{
    ...
    VicrabCrashDoctorParam* receiverParam = [self.params objectAtIndex:0];
    ...
    VicrabCrashDoctorParam* selectorParam = [self.params objectAtIndex:1];
    if(![selectorParam.type isEqualToString:@VicrabCrashMemType_String])
    {
        return nil;
    }
    ...
    for(int paramNum = 0; paramNum < paramCount; paramNum++)
    {
        ...
        if(paramNum < 2)
        {
            VicrabCrashDoctorParam* param = [self.params objectAtIndex:(NSUInteger)paramNum + 2];
            if(param.value != nil)
            {
                if([param.type isEqualToString:@VicrabCrashMemType_String])
                {
...
- (NSString*) descriptionWithParamCount:(int) paramCount
{
    ...
    {
        VicrabCrashDoctorParam* param = [self.params objectAtIndex:(NSUInteger)i];
...
@implementation VicrabCrashDoctor
...
{
    return [report objectForKey:@VicrabCrashField_RecrashReport];
}

- (NSDictionary*) systemReport:(NSDictionary*) report
{
    return [report objectForKey:@VicrabCrashField_System];
}

- (NSDictionary*) crashReport:(NSDictionary*) report
{
    return [report objectForKey:@VicrabCrashField_Crash];
}

- (NSDictionary*) infoReport:(NSDictionary*) report
{
    return [report objectForKey:@VicrabCrashField_Report];
}

- (NSDictionary*) errorReport:(NSDictionary*) report
{
    return [[self crashReport:report] objectForKey:@VicrabCrashField_Error];
}

- (CPUFamily) cpuFamily:(NSDictionary*) report
{
    NSDictionary* system = [self systemReport:report];
    NSString* cpuArch = [system objectForKey:@VicrabCrashField_CPUArch];
...
- (NSString*) mainExecutableNameForReport:(NSDictionary*) report
{
    ...
    return [info objectForKey:@VicrabCrashField_ProcessName];
}

- (NSDictionary*) crashedThreadReport:(NSDictionary*) report
{
    NSDictionary* crashReport = [self crashReport:report];
    NSDictionary* crashedThread = [crashReport objectForKey:@VicrabCrashField_CrashedThread];
    if(crashedThread != nil)
    {
        return crashedThread;
    }

    for(NSDictionary* thread in [crashReport objectForKey:@VicrabCrashField_Threads])
    {
        if([[thread objectForKey:@VicrabCrashField_Crashed] boolValue])
        {
            return thread;
        }
    }
...
{
    NSDictionary* backtrace = [threadReport objectForKey:@VicrabCrashField_Backtrace];
    return [backtrace objectForKey:@VicrabCrashField_Contents];
}

- (NSDictionary*) basicRegistersFromThreadReport:(NSDictionary*) threadReport
{
    NSDictionary* registers = [threadReport objectForKey:@VicrabCrashField_Registers];
    NSDictionary* basic = [registers objectForKey:@VicrabCrashField_Basic];
    return basic;
}
...
- (NSDictionary*) lastInAppStackEntry:(NSDictionary*) report
{
    ...
    {
        NSString* objectName = [entry objectForKey:@VicrabCrashField_ObjectName];
...
{
    NSDictionary* machError = [errorReport objectForKey:@VicrabCrashField_Mach];
    if(machError != nil)
    {
        NSString* exceptionName = [machError objectForKey:@VicrabCrashField_ExceptionName];
        return [exceptionName isEqualToString:@"EXC_BAD_ACCESS"];
    }
    NSDictionary* signal = [errorReport objectForKey:@VicrabCrashField_Signal];
    NSString* sigName = [signal objectForKey:@VicrabCrashField_Name];
...
{
    NSDictionary* crashedThread = [self crashedThreadReport:report];
    NSArray* notableAddresses = [crashedThread objectForKey:@VicrabCrashField_NotableAddresses];
    for(NSDictionary* address in [notableAddresses objectEnumerator])
    {
        NSString* type = [address objectForKey:@VicrabCrashField_Type];
        if([type isEqualToString:@"string"])
        {
            NSString* value = [address objectForKey:@VicrabCrashField_Value];
    ...
    {
        NSString* objectName = [entry objectForKey:@VicrabCrashField_ObjectName];
        NSString* symbolName = [entry objectForKey:@VicrabCrashField_SymbolName];
...
- (VicrabCrashDoctorFunctionCall*) lastFunctionCall:(NSDictionary*) report
{
    VicrabCrashDoctorFunctionCall* function = [[VicrabCrashDoctorFunctionCall alloc] init];
    NSDictionary* lastStackEntry = [self lastStackEntry:report];
    function.name = [lastStackEntry objectForKey:@VicrabCrashField_SymbolName];

    NSDictionary* crashedThread = [self crashedThreadReport:report];
    NSDictionary* notableAddresses = [crashedThread objectForKey:@VicrabCrashField_NotableAddresses];
    ...
    {
        VicrabCrashDoctorParam* param = [[VicrabCrashDoctorParam alloc] init];
        ...
        {
            param.type = [notableAddress objectForKey:@VicrabCrashField_Type];
            NSString* className = [notableAddress objectForKey:@VicrabCrashField_Class];
            NSString* previousClass = [notableAddress objectForKey:@VicrabCrashField_LastDeallocObject];
            NSString* value = [notableAddress objectForKey:@VicrabCrashField_Value];

            if([param.type isEqualToString:@VicrabCrashMemType_String])
            {
                param.value = value;
            }
            else if([param.type isEqualToString:@VicrabCrashMemType_Object])
            {
                param.className = className;
                param.isInstance = YES;
            }
            else if([param.type isEqualToString:@VicrabCrashMemType_Class])
            {
                param.className = className;
                param.isInstance = NO;
            }
...
- (NSString*) zombieCall:(VicrabCrashDoctorFunctionCall*) functionCall
{
...
{
    NSDictionary* stack = [crashedThreadReport objectForKey:@VicrabCrashField_Stack];
    return [[stack objectForKey:@VicrabCrashField_Overflow] boolValue];
}

- (BOOL) isDeadlock:(NSDictionary*) report
{
    NSDictionary* errorReport = [self errorReport:report];
    NSString* crashType = [errorReport objectForKey:@VicrabCrashField_Type];
    return [@VicrabCrashExcType_Deadlock isEqualToString:crashType];
}
...
- (NSString*) diagnoseCrash:(NSDictionary*) report
{
    @try
    {
        NSString* lastFunctionName = [[self lastInAppStackEntry:report] objectForKey:@VicrabCrashField_SymbolName];
        ...
        NSString* crashType = [errorReport objectForKey:@VicrabCrashField_Type];
        if([crashType isEqualToString:@VicrabCrashExcType_NSException])
        {
            NSDictionary* exception = [errorReport objectForKey:@VicrabCrashField_NSException];
            NSString* name = [exception objectForKey:@VicrabCrashField_Name];
            NSString* reason = [exception objectForKey:@VicrabCrashField_Reason]? [exception objectForKey:@VicrabCrashField_Reason]:[errorReport objectForKey:@VicrabCrashField_Reason];
        ...
        VicrabCrashDoctorFunctionCall* functionCall = [self lastFunctionCall:report];
        ...
        {
            uintptr_t address = (uintptr_t)[[errorReport objectForKey:@VicrabCrashField_Address] unsignedLongLongValue];
            if(address == 0)
            {
    ...
    @catch (NSException* e)
    {
        ...
        if(symbols)
        {
            return [NSString stringWithFormat:@"No diagnosis due to exception %@:\n%@\nPlease file a bug report to the VicrabCrash project.", e, symbols];
        }
        return [NSString stringWithFormat:@"No diagnosis due to exception %@\nPlease file a bug report to the VicrabCrash project.", e];
    }
}
