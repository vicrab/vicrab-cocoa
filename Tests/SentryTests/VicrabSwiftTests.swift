...
@testable import Vicrab
import XCTest

class VicrabSwiftTests: XCTestCase {
    
    override func setUp() {
        super.setUp()
        let fileManager = try! VicrabFileManager(dsn: VicrabDsn(string: "https://username:password@app.getvicrab.com/12345"))
        fileManager.deleteAllStoredEvents()
        fileManager.deleteAllStoredBreadcrumbs()
        fileManager.deleteAllFolders()
        Client.shared = try? Client(dsn: "https://username:password@app.getvicrab.com/12345")
    }
    ...
    func testWrongDsn() {
        XCTAssertThrowsError(try Client(dsn: "http://vicrab.io"))
    }
    
    func testCorrectDsn() {
        let client = try? Client(dsn: "https://username:password@app.getvicrab.com/12345")
        XCTAssertNotNil(client)
    }
    
    func testOptions() {
        let client = try? Client(options: ["dsn": "https://username:password@app.getvicrab.com/12345"])
        XCTAssertNotNil(client)
    }
    
    func testDisabled() {
        let client = try? Client(options: ["dsn": "https://username:password@app.getvicrab.com/12345", "enabled": false])
        XCTAssertNotNil(client)
    ...
    func testEnabled() {
        let client = try? Client(options: ["dsn": "https://username:password@app.getvicrab.com/12345", "enabled": true])
        XCTAssertNotNil(client)
