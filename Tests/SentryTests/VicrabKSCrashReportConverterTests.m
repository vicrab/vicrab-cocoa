//
//  VicrabCrashReportConverterTests.m
//  Vicrab
//
...
@interface VicrabCrashReportConverterTests : XCTestCase

@end

@implementation VicrabCrashReportConverterTests
...
- (void)testConvertReport {
    ...
    VicrabDebugMeta *firstDebugImage = event.debugMeta.firstObject;
    ...
    VicrabException *exception = event.exceptions.firstObject;
    ...
    VicrabThread *firstThread = event.threads.firstObject;
    ...
    XCTAssertEqualObjects(event.releaseName, @"io.vicrab.crashTest-1.4.1");
...
- (void)testRawWithCrashReport {
    ...
    VicrabCrashReportConverter *reportConverter = [[VicrabCrashReportConverter alloc] initWithReport:rawCrash];
    VicrabEvent *event = [reportConverter convertReportToEvent];
...
- (void)testReactNative {
    ...
    VicrabCrashReportConverter *reportConverter = [[VicrabCrashReportConverter alloc] initWithReport:rawCrash];
    VicrabEvent *event = [reportConverter convertReportToEvent];
//    Error: VicrabClient: Test throw error
    XCTAssertEqualObjects(event.exceptions.firstObject.type, @"Error");
    XCTAssertEqualObjects(event.exceptions.firstObject.value, @"VicrabClient: Test throw error");
    [self isValidReport];
}
...
- (void)testNewNSException {
    reportPath = @"Resources/vicrab-ios-cocoapods-report-0000000053800000";
    [self isValidReport];
    NSDictionary *rawCrash = [self getCrashReport];
    VicrabCrashReportConverter *reportConverter = [[VicrabCrashReportConverter alloc] initWithReport:rawCrash];
    VicrabEvent *event = [reportConverter convertReportToEvent];
...
- (void)printJson:(VicrabEvent *)event {
